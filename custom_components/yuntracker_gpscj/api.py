...
def fake_gpscj_login_and_get_device_position(username: str, password: str, user_id: str, device_id: str):
    """Fake implementation of the function for testing."""
    import random
    from datetime import datetime, timedelta

    return {
        "baiduLat": -1.793659 + random.uniform(-0.01, 0.01),
        "baiduLng": -1.55398 + random.uniform(-0.01, 0.01),
        ...
        "dataType": "3",
        "device_utc_date": (datetime.now() - timedelta(minutes=random.randint(0, 60))).isoformat() + "Z",
        ...
        "id": 2017806,
        "isStop": 1,
        "latitude": -1.793659 + random.uniform(-0.01, 0.01),
        "locationID": "1",
        "longitude": -1.55398 + random.uniform(-0.01, 0.01),
        "model": "198",
        "modelName": "CJPSN",
        "name": "CJPSN-00123",
        "ofl": "0",
        "server_utc_date": (datetime.now() - timedelta(minutes=random.randint(0, 60))).isoformat() + "Z",
        ...
        "state": 1,
        "status": "Stop",
        "stop_time": (datetime.now() - timedelta(days=random.randint(0, 2))).isoformat() + "Z",
        "stop_time_minute": random.randint(0, 60),
    }
